...
```{r  Dealing with missing values: Mean based insertion for continuous vars}
...
> NaiveBayes does not produce good results even on the training set. In this data, one of the classes, that is 1, is more important than the other class. So, checking the performance of the model using accuracy is not the best idea. 

> To identify 1s more, the specificity of the model needs to be very high. And 
